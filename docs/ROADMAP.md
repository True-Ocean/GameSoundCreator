...
- [x] BGM: シーン2階層（画面／プレイ中／結果）→ 詳細・音色・雰囲気・長さ・展開。`genreId` は内部既定
...
- [x] 共有シートでのファイルエクスポート
- [x] 基本的なエラー・空状態表示
- [x] 再生バー（現在位置／長さ）。再生中は再生ボタンが停止に切替
...
- [x] **長さ連動の展開** — 8=くり返し／16=起承／24=起承転結（メロディ＋アレンジ区間差）
...
| 2026-08-01 | BGM長さで展開が変わる（8くり返し／16起承／24起承転結） | 尺だけでなくメロディ・アレンジが区間で変化 |
...
2. 実機で BGM／SE スタジオの一連操作・生成時間を確認
3. （任意）**3.5-M2** — コール＆レスポンス／フィルの厚み
