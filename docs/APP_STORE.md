...
| 課金・広告 | 未実装 |
| 利用規約・プライバシーポリシー | 未作成 |
| Privacy Manifest（`PrivacyInfo.xcprivacy`） | 未作成（`@AppStorage`＝UserDefaults 利用あり） |
| 方針 | 完全ローカル・手続き生成・オフライン・クラウドAI不使用 |
| ブランド | 起動画面に ChatNoir Studio |

品質は概ね満足できる段階。これから必要なのは **法務・アカウント・ストア素材・（任意）課金・審査提出** です。
...
審査時に **HTTPS で開ける**こと。App Store Connect とアプリ内の両方から辿れるようにする。

| 種別 | 必須？ | 内容 |
|------|--------|------|
| **Privacy Policy URL** | 必須 | 収集の有無、端末内保存、第三者送信、連絡先。オフライン方針を明記 |
| **Support URL** | 必須 | 問い合わせ方法（簡易ページ＋メールで可） |
...
- [ ] 設定などから **利用規約・プライバシーポリシー**を開ける
- [ ] 生成音の権利・手続き生成である旨をアプリ内でも確認できる
- [ ] **`PrivacyInfo.xcprivacy`** を追加  
  - UserDefaults（`@AppStorage`）など Required Reason API を宣言  
  - データ収集がなければ収集タイプも実態に合わせて記載
- [ ] App Store Connect の **暗号化（輸出コンプライアンス）** 質問に正確に回答  
  - HTTPS のみ等の一般ケースは免除になりやすい。音響合成自体は通常問題になりにくい
- [ ] **開発用 UI**（設定→開発用・レガシー画面）を審査ビルドで隠すか削除
- [ ] 実機で一連操作確認（BGM／SE／保存／書き出し／共有／バックグラウンド）
- [ ] **Release Archive** で動作確認（Debug 専用コードがないこと）
- [ ] Marketing Version を `1.0.0` 等に上げる。提出のたびに **Build 番号をインクリメント**
...
- [ ] 年齢レーティングアンケート
- [ ] **App Privacy（栄養成分表示）**  
  - 現状方針なら「データを収集しない」に近い。広告・解析を入れたら再申告
...
| iPhone スクリーンショット | 未着手。主サイズは **6.9 インチ系**（例: 1320×2868 等）。実 UI を写す |
| 枚数 | 3〜数枚推奨（ホーム／BGM／SE／ライブラリなど） |
| プレビュー動画 | 任意（最大約 30 秒） |
...
- [ ] **審査に提出**
- [ ] リジェクト時は指摘を修正し、**新しいビルド**で再提出（同じビルド番号の使い回しに注意）
- [ ] 承認後、手動公開 or 自動公開を選択
...
1. Apple Developer Program ＋ 税務・銀行  
2. プライバシーポリシー／サポート URL の公開  
3. 収益モデル確定（無料のみ先行か、Pro 込みか）  
4. Privacy Manifest ＋ 審査向け仕上げ（開発メニュー隠し、`1.0.0`）  
5. スクリーンショットとストア説明文  
