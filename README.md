...
| 短期 | 中期 | 長期 |
|------|------|------|
| 自作カードバトル用のBGM・SEを量産できる | ゲームタイプ・シーン・用途などを選ぶだけで試聴・書き出しできるiOSアプリ | App Storeで広告 or 買い切り販売 |
...
1. **製品を三層で考える**  
   - **アプリ層:** 意図ウィザード（BGM:ゲームタイプ×シーン／SE:カテゴリ→用途／雰囲気・長さ）、試聴、書き出し、課金  
   - **Intent Mapper:** ユーザー意図 → Recipe  
   - **エンジン層:** Recipe → 音声バッファ / ファイル  

2. **最初のゲームタイプはカードバトル中心**  
   BGM では RPG／パズルも選択可（音色寄り）。SE は用途カタログを拡充する。
...
4. BGMはゲームタイプとシーン、SEはカテゴリと用途、雰囲気・長さを選んで再生  
